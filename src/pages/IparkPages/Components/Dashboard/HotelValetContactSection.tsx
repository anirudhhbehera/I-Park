...
export default function HotelValetContactSection() {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        const formattedData = response.data.map((item) => ({
          hotelName: item.hotelId?.name || 'N/A',
          BranchName: item.branchId?.name || 'N/A',
          valleyBoy: item.name || 'N/A',
          contact: item.phone || 'N/A'
        }));

        setData(formattedData);
        setPermissionsState(formattedData.map(() => false));
        setLoading(false);
      } else {
      ...
      setLoading(false);
      console.error('Error fetching valet contact data:', error);
    }
  };
  ...
  const exportToPDF = PDFExporter({
    ...
    columns: [
      ...
      { Header: 'Valley Boy', accessor: 'valleyBoy' },
      ...
      <div className="relative flex h-[400px] flex-col rounded-xl bg-white p-6 shadow-md">
        ...
        <div className="max-h-[500px] flex-1 overflow-y-auto pr-2">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 text-sm">
              {paginatedData.map(
                ({ hotelName, BranchName, valleyBoy, contact }, index) => (
                  <tr
                    key={index}
        ...
        <div className="mt-4">
          <StyledTablePagination>
            <TablePagination
              rowsPerPageOptions={[{ label: 'All', value: -1 }, 5, 10, 25, 100]}
              component="div"
              count={data.length}
              rowsPerPage={rowsPerPage}
              page={page}
